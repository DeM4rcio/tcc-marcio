...
\vspace{0.5cm}
\noindent \textbf{Questão Específica 3: Qual é o nível de modularidade e o impacto da complexidade arquitetural no código-fonte Java ao longo das entregas?}

Para avaliar especificamente a arquitetura e a modularidade da aplicação, serão adotadas as métricas de estrutura consolidadas pela ferramenta \textit{CBR Insight}, cujos fundamentos teóricos se baseiam na literatura clássica de engenharia de software:

\begin{itemize}
    \item \textbf{Métrica 3.1 -- Linhas de Código (LOC):} Métrica fundamental de volume que quantifica o tamanho físico do código-fonte da classe. .
    
    \item \textbf{Métrica 3.1 -- Peso dos Métodos (\textit{WMC-Unweighted}):} Consiste na contagem absoluta (não ponderada) de métodos implementados em uma classe..
    
    \item \textbf{Métrica 3.1 -- Resposta para uma Classe (RFC):} Mensura a cardinalidade do conjunto de resposta de uma classe, ou seja, o número total de métodos locais somado ao número de métodos externos diretamente invocados por eles.  \cite{chidamber1994}.
    
    \item \textbf{Métrica 3.1 -- Acoplamento entre Objetos (CBO):} Representa a quantidade de classes independentes às quais uma determinada classe está acoplada (seja por atributos, parâmetros de métodos, retornos, herança ou chamadas estáticas).  \cite{chidamber1994}.
