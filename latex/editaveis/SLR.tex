...
Neste  capítulo será apresentado o estudo da Revisão Estruturada, a fim de levantar o embasamento bibliográfico, respondendo a pergunta principal de pesquisa por intermédio das perguntas secundárias de pesquisa. 
...
As bases de dados acadêmicas indexam uma grande quantidade de artigos, de diferentes áreas de conhecimento.  Dessa forma, requer de o uso de um protocolo para a recuperação adequada dos mesmos a serem analisados dentro do escopo da pesquisa. Tendo em vista o grau de sensibilidade da \textit{string} de busca, qualquer grau de incoerência pode acarretar em um número excessivo de artigos sem relação com o tema da pesquisa, ou o inverso, ocorrer de ter um número insuficiente de estudos para responder à questão de pesquisa.

Por esta razão, o protocolo PICO foi utilizado para guiar a elaboração de uma string de busca adequada às necessidades desta monografia. Entretanto, pelo protocolo ter origem da área de pesquisa da medicina, sendo necessário uma adequação para a sua aplicação na área de Engenharia de Software. Uma representação visual que facilita a compreensão do protocolo pode ser vista na Figura \ref{pico-adaptado}.
...
Com a string de busca criada, foram definidos os critérios de inclusão e exclusão visando selecionar apenas os artigos mais pertinentes ao escopo da pesquisa. A partir da leitura do Título, Resumo e palavras-chaves dos artigos extraídos da string de busca, foram rejeitados aqueles que não atendia aos critérios de inclusão ou se enquadrava aos critérios de exclusão, e aceitados aqueles que atendia a todos os critérios de inclusão. Posteriormente a partir do estudos selecionados foi preenchido o formulário de extração a fim de atender as perguntas de pesquisa secundárias. O Protocolo completo está definido na Tabela.

Ao todo foram analisado 501 artigos resultantes da string de busca, onde 50 desses foram selecionados e lidos de maneira integral. Esses artigos foram obtidos na base de dados Scopus no dia 14 de maio de 2026.Os artigos selecionados estão dispostos na Tabela~\ref{tab-artigos-slr}.



\todo[inline, color=pink]{Exemplo de nota ao longo do texto, na cor rosa.

 A calibração da \textit{string} de busca era determinado se o artigo Tools challanges  estava presente na base de busca e isso identificava que os resultados estavam dentro do escopo da pesquisa. 

\todo[inline, color=pink]{Incluir as referencias dos artigo}
